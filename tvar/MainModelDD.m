...
if RunEstimation
    ...
    %      pi       m       ts      liq     safe   DD
    Ctr =[
        1        0       0        0       0     0   ;%1: Inflatiom
        1        0       0        0       0     0   ;%2: Inflation Expectations
        1        1       0       -1      -1     0   ;%3: Short term rate (3m TBILL)
        1        1       0       -1      -1     0   ;%4: Expected Short Rate
        1        1       1       -1      -1     0   ;%5: Long Rate (20y TBOND)
        1        1       1        0      -1     0   ;%6: AAA
        1        1       1        0       0     0   ;%7: BAA
        0        0       0        0       0     1   ;%DD
        ];
    ...
    df0tr = 100;
    %              pi^*      r^*        ts^*      liq        safe       DD
    SC0tr =    ([   2     1/sqrt(2)      1     1/sqrt(4)     1/sqrt(4)   1 ]).^2/400;%var(diff(Trend));
    S0tr =      [   2        1.5         1        .25         .75        8 ]';
    P0tr = diag([   1         1          1         1           1         1]);
    Psi =                  [ 2    1     1    .5      1        1        1      1   ];
    %                       pi   Epi   bill  Eill   bond     AAA      BAA     DD
    S0cyc = zeros(n*p,1);
    ...
    save(OutputName,'CommonTrends','Trends','TrendsReal','Cycles','AA','QQ','CC','RR','LogLik','SS0','P_acc','Ndraws','Discard','SC0tr','S0tr','P0tr','df0tr','Psi','Time','Y','y','Mnem','Lambda')
...
R_bar     =  M_bar - Cy_bar;
D_bar     =  squeeze(CommonTrends(:,6,:));
LD_bar     =  ones(size(Time))*Lambda.*squeeze(CommonTrends(:,6,:));
...
qD_bar     = sD_bar(:,ceil(Quant*M));
qLD_bar    = sLD_bar(:,ceil(Quant*M));


save OutModDDforCharts Time qD_bar qLD_bar qR_bar qCy_bar qM_bar qPi_bar  qTs_bar qSafe_bar qLiq_bar y
...
f = figure;
PlotStatesShaded(Time,qD_bar)
filename=strcat(['./',FigSubFolder,'/Dbar.pdf']);
printpdf(f,filename);
title('D^*')
...
axis([-inf inf 0 12.5])
hold off
filename=strcat(['./',FigSubFolder,'/Dbar_obs.pdf']);
printpdf(f,filename);
title('D^* and D')
...
axis([-inf inf 0 12.5])
hold off
filename=strcat(['./',FigSubFolder,'/DD.pdf']);
printpdf(f,filename);
title('D')
...
f = figure;
histogram(Lambda,30,'Normalization','pdf')
filename=strcat(['./',FigSubFolder,'/HistLambda.pdf']);
print('-dpdf', filename);
...
disp('Change in DD')
dD = D_bar(end,:)-D_bar(tmax,:);
disp(quantile(dD,Quant))

disp('Change in Lambda DD')
dLD = LD_bar(end,:)-LD_bar(tmax,:);
disp(quantile(dLD,Quant))
