...
RunEstimation = 1;
OutputName = 'OutputModel3ZLBobs';
FigSubFolder = 'FiguresModel3ZLBobs';
...
M = size(sStates,3);
qStates = sStates(:,:,ceil(Quant*M));
qTrends = sTrends(:,:,ceil(Quant*M));
qTrendsReal = sTrendsReal(:,:,ceil(Quant*M));
 
 
 
Pi_bar    =  squeeze(States(:,1,:));
M_bar     =  squeeze(States(:,2,:));
Ts_bar    =  squeeze(States(:,3,:));
Liq_bar   =  squeeze(States(:,4,:));
Safe_bar  =  squeeze(States(:,5,:));
...
save OutMod3ZLBobsforCharts Time qR_bar qCy_bar qM_bar qPi_bar qTs_bar qSafe_bar qLiq_bar y
...
for j = 1:size(Ctr,2)
    f=figure(100+j)
    PlotStatesShaded(Time,squeeze(qStates(:,j,:)))
    hold on
    plot(Time,Ytr(:,j))
    hold off
    title(num2str(j))
...
for j = 1:size(Ctr,1)
    ...
    title(Mnem{j})
    subplot(2,1,2)
    PlotStatesShaded(Time,squeeze(qStates(:,r+j,:)))
...
f = figure;
histogram([sqrt(squeeze(QQ(1,1,:)))]*20,30,'Normalization','probability')
axis([0 4 0 inf])
