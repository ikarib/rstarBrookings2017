...
RunEstimation = 1;
OutputName = 'OutputModel3Loose';
FigSubFolder = 'FiguresModel3Loose';
...
if RunEstimation
    ...
    for jm = 1:Ndraws
        ...
        if ~isnan(notrend)
            temp = S0(notrend) + randn(length(notrend),1)*sqrt(length(notrend));
...
save OutMod3LooseforCharts Time qR_bar qCy_bar qM_bar qPi_bar qTs_bar qSafe_bar qLiq_bar y
...
hold on
[dens,grid] = ksdensity([sqrt(squeeze(QQprior(1,1,:)))]*20,0:.01:20);
plot(grid,dens);
hold off
axis([0 10 0 inf])
...
hold on
[dens,grid] = ksdensity([sqrt(squeeze(QQprior(2,2,:)))]*20,0:.01:20);
plot(grid,dens);
hold off
axis([0 10 0 inf])
...
hold on
[dens,grid] = ksdensity([sqrt(squeeze(QQprior(3,3,:)))]*20,0:.01:20);
plot(grid,dens);
hold off
axis([0 10 0 inf])
...
hold on
[dens,grid] = ksdensity([sqrt(squeeze(QQprior(4,4,:)))]*20,0:.01:20);
plot(grid,dens);
hold off
axis([0 10 0 inf])
...
hold on
[dens,grid] = ksdensity([sqrt(squeeze(QQprior(5,5,:)))]*20,0:.01:20);
plot(grid,dens);
hold off
axis([0 10 0 inf])
